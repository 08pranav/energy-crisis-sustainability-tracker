      ...
      <div className="w-full flex-1 max-w-[1400px] mt-40 relative flex flex-col px-4 pointer-events-auto h-full pb-10 gap-6 overflow-y-auto custom-scrollbar">
        ...
        <div className="grid grid-cols-12 gap-6 min-h-[450px]">
                ...
                {[
                  ...
                  <div key={i} className="flex justify-between items-end border-b border-slate-800 pb-2">
                    <div className="space-y-1">
                      <div className="text-[9px] text-gray-500 font-mono tracking-widest uppercase">{item.event}</div>
                      <div className={`text-[10px] font-black font-mono ${item.color}`}>{item.risk}</div>
                    </div>
                    <div className="text-lg font-mono font-bold tracking-tighter">{item.val}</div>
                  </div>
                ))}
              </div>
            </HudPanel>

            <div className="bg-red-500/5 border border-red-500/20 p-5 relative group overflow-hidden mt-4">
               ...
               <div className="text-3xl font-black font-mono relative z-10 text-white">+112% <span className="text-[10px] font-normal text-gray-500 tracking-normal italic uppercase">Volatility Spike</span></div>
          ...
          <div className="col-span-12 lg:col-span-8 flex flex-col h-full">
            <HudPanel title="Conflict vs Sustainability Flux // METRICS" className="h-full flex flex-col">
              <div className="flex justify-end items-center mb-4">
                <div className="flex gap-4 font-mono text-[9px] tracking-widest font-bold">
                   <span className="flex items-center gap-1 text-emerald-500"><span className="w-2 h-2 bg-emerald-500 rounded-sm"></span> RENEWABLES</span>
                   <span className="flex items-center gap-1 text-red-500"><span className="w-2 h-2 border border-red-500 bg-red-500/20 rounded-sm"></span> FOSSIL</span>
              ...
              <div className="flex-1 w-full min-h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={data} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                    ...
                    <Area type="monotone" dataKey="fossil" stroke="#ef4444" fill="#ef4444" fillOpacity={0.1} strokeWidth={1} strokeDasharray="4 4" />
              ...
              <div className="mt-4 pt-4 border-t border-slate-800 flex justify-between text-[9px] font-mono text-slate-500 font-bold tracking-widest">
